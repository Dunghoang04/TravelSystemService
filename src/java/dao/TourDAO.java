...
public class TourDAO extends DBContext implements ITourDAO {
    ...
    public int getTotalToursByTravelAgent(int id) throws SQLException {
        ...
        PreparedStatement ps = null;
        ResultSet rs = null;
        String sql = "  SELECT COUNT(*) FROM Tour where travelAgentID = ?";
        try {
            conn = getConnection(); // Establish database connection
            ps = conn.prepareStatement(sql); // Prepare SQL query
            ps.setInt(1, id);
    ...
    public Vector<Tour> getToursWithFilters(String budget, String departure, String destination,
        ...
        if (destination != null && !destination.isEmpty()) {
            sql.append(" AND endPlace LIKE ?"); // Add destination filter
        }
     ...
     */
     @Override
    public boolean updateQuantityAfterBooking(int tourID, int totalPeople) {
        String sql = "UPDATE Tour SET quantity = quantity - ? WHERE tourID = ? AND quantity >= ?";
        Connection conn = null;
        ...
        try{
            ...
            stmt.setInt(3, totalPeople); // đảm bảo còn đủ chỗ
    ...
    public boolean hasBookings(int tourId) throws SQLException {
        ...
        PreparedStatement ps = null;
        ResultSet rs = null;
        String sql = "SELECT COUNT(*) FROM BookDetail WHERE tourID = ?";
        try {
            conn = getConnection();
            ps = conn.prepareStatement(sql);
            ps.setInt(1, tourId);
    ...
    @Override
    public boolean updateQuantityAfterCancel(int tourID, int numberHuman) throws Exception {
