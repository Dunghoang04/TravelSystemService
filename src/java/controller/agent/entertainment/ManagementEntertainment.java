...
public class ManagementEntertainment extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Entertainment> listEnt;
        EntertainmentDAO entDao = new EntertainmentDAO();
        String findByType = request.getParameter("type");
        if (findByType != null && !findByType.isEmpty()) {
            listEnt = entDao.getEntertainmentByType(findByType);
        ...
        listEnt = entDao.getListEntertainment();
        session.setAttribute("listEnt", listEnt);
        session.setAttribute("status", findByType);
        request.getRequestDispatcher("view/agent/entertainment/AgentEntertainment.jsp").forward(request, response);
