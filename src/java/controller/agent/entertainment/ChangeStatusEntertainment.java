...
public class ChangeStatusEntertainment extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (serviceIdParam.isEmpty()) {
            response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Service ID is missing or contains only spaces");
            return;
        }
        ...
        try {
            ...
            if (serviceId <= 0) {
                throw new NumberFormatException("Mã dịch vụ phải là số dương");
            }
        } catch (NumberFormatException e) {
            response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Sai định dạng mã dịch vụ: " + e.getMessage());
            return;
        }
        ...
        try {
            ...
            if (!isSuccess) {
                throw new SQLException("Failed to update status for service ID: " + serviceId);
            }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        response.sendError(HttpServletResponse.SC_METHOD_NOT_ALLOWED, "POST method not supported for this servlet");
    }
