...
/**
 * Servlet for adding a new admin user to the TravelSystemService.
 * Handles form submission, input validation, and user insertion into the database.
 * Forwards to the addAdmin.jsp page for rendering the form or displaying results.
 *
 * @author Hà Thị Duyên
 */
...
public class AddAdmin extends HttpServlet {
    ...
    /**
     * Processes both GET and POST requests for adding an admin.
     * Displays the add admin form or processes form submission with validation and database insertion.
     *
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        UserDAO dao = new UserDAO(); // Initialize UserDAO for database operations
        String service = request.getParameter("service"); // Get service parameter

        // If no service specified, show the add admin form
        ...
        if (service.equals("addAdmin")) {
            ...
            validateInput(request, dao, firstName, lastName, password, repassword, phone, dobStr, gender, address, gmail);

            // Proceed with admin creation if no validation errors
            try {
                if (request.getAttribute("lastNameError") == null &&
                    ...
                    // Parse date of birth and set create/update dates
                    ...
                    dao.insertUser(newAdmin); // Insert admin into database
                    request.setAttribute("successMessage", "Thêm quản trị viên thành công!"); // Set success message
                }
                request.getRequestDispatcher("/view/admin/addAdmin.jsp").forward(request, response); // Forward to JSP
            } catch (SQLException e) {
                e.printStackTrace();
                request.setAttribute("generalError", "Thêm quản trị viên thất bại. Vui lòng thử lại."); // Set SQL error message
                request.getRequestDispatcher("/view/admin/addAdmin.jsp").forward(request, response); // Forward to JSP
            } catch (Exception e) {
                e.printStackTrace();
    ...
    /**
     * Validates form input data for adding an admin.
     ...
     * @param dao         The UserDAO instance for checking Gmail existence
     ...
     */
    private void validateInput(HttpServletRequest request, UserDAO dao, String lastName, String firstName, 
            String password, String repassword, String phone, String dobStr, String gender, String address, String gmail) {
        // Validate last name
        ...
        if (dobStr == null || dobStr.trim().isEmpty()) {
            ...
            try {
                ...
                if (dob.isAfter(today)) {
                    ...
                    request.setAttribute("dobError", "Quản trị viên phải đủ 18 tuổi trở lên!"); // Underage error
    ...
    public String getServletInfo() {
        return "Handles admin addition with individual error reporting using setAttribute.";
    }
}