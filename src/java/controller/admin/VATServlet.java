...
public class VATServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (service == null || service.equals("listVAT")) {
            ...
            try {
                vatDAO.updateExpiredVATs(); // Cập nhật trạng thái VAT hết hiệu lực
                Vector<VAT> vatList = vatDAO.getAllVAT(sql);
                request.setAttribute("vatList", vatList);
                request.setAttribute("today", new java.util.Date()); // Truyền ngày hiện tại
            ...
            try {
                ...
                if (vat != null) {
                    int newStatus = vat.getStatus() == 1 ? 0 : (vat.getStatus() == 0 ? 1 : 2); // Chuyển tuần hoàn: 1->0, 0->1, 2->2
                    vatDAO.changeVATStatus(vatID, newStatus);
                    request.setAttribute("successMessage", "Thay đổi trạng thái VAT thành công!");
