    ...
    <body class="sb-nav-fixed">
        ...
        <script>
                                          ...
                                          function loadMessages() {
                                              $.get('${pageContext.request.contextPath}/ChatbotServlet?ajax=true', function (data) {
                                                  const chatBody = document.getElementById('chatbot-messages');
                                                  chatBody.innerHTML = '';
                                                  if (data.length === 0) {
                                                      ...
                                                      data.forEach(message => {
                                                          appendMessage(message.text, message.sender === 'user' ? 'user-message' :
                                                                  message.sender === 'bot' ? 'bot-message' : 'staff-message');
                                                      });
                                                  }
                                              }).fail(function (xhr, status, error) {
                                                  console.error('Lỗi khi tải tin nhắn:', error, xhr.responseText);
                                                  const chatBody = document.getElementById('chatbot-messages');
                                          ...
                                          function sendStaffResponse() {
                                              var text = document.getElementById('staff-input').value.trim();
                                              if (text !== '') {
                                                  $.post('${pageContext.request.contextPath}/ChatbotServlet', {
                                                      action: 'sendMessage',
                                                      sender: 'staff',
                                                      text: text
                                                  }, function (data) {
                                                      ...
                                                      console.error('Lỗi khi gửi tin nhắn:', error, xhr.responseText);
